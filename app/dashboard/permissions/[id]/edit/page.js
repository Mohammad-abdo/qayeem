...
export default function EditPermissionPage() {
  ...
  const fetchPermission = async () => {
    try {
      setLoading(true)
      const response = await adminPermissionsAPI.getById(params.id)
      const permission = response.data.permission
      setFormData({
        name: permission.name || '',
        nameAr: permission.nameAr || '',
        resource: permission.resource || '',
        resourceAr: permission.resourceAr || '',
        action: permission.action || '',
        actionAr: permission.actionAr || '',
        description: permission.description || '',
        descriptionAr: permission.descriptionAr || '',
      })
    } catch (error) {
      toast.error('فشل تحميل بيانات الصلاحية')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSaving(true)
      await adminPermissionsAPI.update(params.id, formData)
